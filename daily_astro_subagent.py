...
class AstroSignExtraction:
    #数据类，从用户问题中提取星座的结果
    user_sign: str | None
    mentioned_signs: list[str]
    other_signs: list[str]
...
class DailyAstroSearchTool:
    ...
    def search(self, sign, topic, query):
        ...
        prompt = (
            "Search the web for today's daily horoscope context for the requested zodiac sign. "
            "Return compact JSON with keys content and sources. Sources must contain title and url when available. "
            "Keep the wording reflective and non-deterministic."
        )
...
class DailyAstroSubagent:
    ...
    def extract_user_sign(self, query):
        ...
        for sign, _ in mentions:
            if sign not in seen:
                seen.append(sign)
        if not mentions:
            return AstroSignExtraction(user_sign=None, mentioned_signs=[], other_signs=[])
        if len(seen) == 1:
            return AstroSignExtraction(user_sign=mentions[0][0], mentioned_signs=seen, other_signs=[])

        user_sign = next(
            (sign for sign, idx in mentions if idx > 0 and tokens[idx - 1] in USER_CUES),
            mentions[0][0],
        )
        return AstroSignExtraction(user_sign=user_sign, mentioned_signs=seen, other_signs=[sign for sign in seen if sign != user_sign])
    ...
    def _summarize_retrieved_horoscope(self, sign, topic, query, retrieved):
        ...
        prompt = (
            "You are an Assistant Astrologer for a Tarot Reader. "
            "Use the retrieved context, summarize the zodiac horoscope in 1-2 sentences."
        )
        payload = {
            "sign": sign,
            "topic": topic,
