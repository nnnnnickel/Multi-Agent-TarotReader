...
from tarot_reader_agent import DEFAULT_SESSION_ID, TarotReaderAgent
...
class TarotWebHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        try:
            ...
            if request_path == "/api/exit":
                ...
                TarotReaderAgent().reset_memory(session_id)
                self._send_json(
                    {
                        "status": "success",
                        "message": "会话已结束，记忆已清空。",
    ...
    def _run_reading(self, payload: dict[str, object]) -> dict[str, object]:
        ...
        agent = TarotReaderAgent(use_skills=bool(payload.get("use_skills", True)))
