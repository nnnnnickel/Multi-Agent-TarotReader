...
QUIT_COMMANDS = {"/q", "/quit", "q", "quit", "exit"}
...
def print_reading(result: dict[str, Any], show_json: bool = False) -> None:
    if show_json:
...
def run_followup(agent: TarotReaderAgentP1, session_id: str, followup_query: str, extra_cards: list[str] | None = None) -> dict[str, Any]:
    return agent.run_pipeline(user_query=followup_query, cards=extra_cards or None, session_id=session_id)
...
def interactive_followups(agent: TarotReaderAgentP1, session_id: str, show_json: bool = False) -> None:
    ...
    print("Press Enter or type /quit to finish. Type /history to view session memory. Type /reset to clear memory.")
    while True:
