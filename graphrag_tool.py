...
class GraphRAGResult:
    #GraphRAG检索结果
    card_meanings: list[dict[str, object]]
    element_analysis: dict[str, object]
    astro_associations: list[dict[str, object]]
...
class GraphRAGTool:
    ...
    def retrieve(self, cards, topic = "general") -> dict[str, object]:
        ...
        if self.entities is None or self.relationships is None:
            return self._failed_result(cards, topic, self.load_error or "GraphRAG parquet tables 不可用").to_dict()
    ...
    def _load_tables(self):
        ...
        try:
            ...
            entities_path = self.graphrag_project_path / "entities.parquet"
            relationships_path = self.graphrag_project_path / "relationships.parquet"
            reports_path = self.graphrag_project_path / "community_reports.parquet"
            if not entities_path.exists() or not relationships_path.exists():
                self.load_error = f"Missing parquet files under {self.graphrag_project_path}."
                return
    ...
    def _retrieve_from_tables(self, cards, topic):
        ...
        for idx, raw_card in enumerate(cards):
            ...
            element = self._infer_element(title)
            if element != "未知":
    ...
    def _strip_card_orientation(card):
        ...
        orientation = r"(?:reversed|reverse|rev|upright|正位|逆位)"
    ...
    def _two_hop_chains(self, title: str, one_hop: list[dict[str, object]]):
        """
        获取links
        Input:
            title: str
    ...
    def _astro_links(self, raw_card, title, one_hop):
        ...
        for rel in one_hop:
            ...
            if source.upper() in ASTRO_TERM or target.upper() in ASTRO_TERM:
                links.append(
                    {
                        "card": raw_card,
    ...
    def _element_analysis(self, counter: Counter[str], total_cards: int) -> dict[str, object]:
        """
